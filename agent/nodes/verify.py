...
def verify(state: AgentState, deps: Deps) -> dict:
    ...
    # Оговорка обязана различать два очень разных исхода, иначе мнение рецензента
    # читается как установленный факт. Живой прогон 14.09 показал, во что это
    # выливается: под ответом стояло «Уверенность: высокая», а сразу под ним —
    # «часть утверждений не подтверждена источниками» со списком ВСЕХ ключевых
    # утверждений, каждое из которых снабжено координатой из доказательной базы и
    # прошло механическую сверку. Ответ опровергал сам себя, причём неправдиво.
    #
    # Механическое нарушение — ссылка вне базы, вычеркнутое утверждение — это
    # факт, проверенный кодом, и говорить о нём надо твёрдо. Замечание рецензента
    # при чистой механике — это несогласие модели, и называть его «не
    # подтверждено источниками» нельзя: источники как раз указаны и прочитаны.
    mechanical = bool(unknown) or bool(state.struck_claims)
    note_lines = [""]
    if mechanical:
        note_lines.append("Проверка ответа: часть утверждений не подтверждена источниками.")
        note_lines += [f"- ссылка на источник вне доказательной базы: {u}" for u in unknown[:3]]
        if state.struck_claims:
            note_lines.append(
                f"- вычеркнуто утверждений без основания: {state.struck_claims}")
        note_lines += [f"- {f['claim']}" for f in findings[:5]]
    elif findings:
        note_lines.append(
            "Проверка ответа: механическая сверка координат пройдена — все ссылки ведут "
            "на прочитанные источники. Рецензент при этом не нашёл подтверждения "
            "следующим утверждениям, проверьте их отдельно:")
        note_lines += [f"- {f['claim']}" for f in findings[:5]]
    updates["answer"] = state.answer + "\n" + "\n".join(note_lines)
