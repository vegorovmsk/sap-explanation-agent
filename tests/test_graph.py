...
def main() -> int:
    ...
    print("\nСловарь параметров читается у стенда, а не пишется руками")
    # Здесь лежал словарь из одиннадцати записей, написанный по памяти. Стенд
    # публикует это соответствие сам — column_translation в его config.yaml, 54
    # записи. Цена рукописного словаря измерена: в нём не было «Блок 3, км →
    # min_caliber_block», и обход не видел, что ТР-ЭКС п. 3.5 требует формировать
    # блоки по калибрам, табл. 12 даёт колонку, а demo/extrusion.py читает только
    # min_common_block и min_color_block. Именно об этом спрашивает кейс B3.
    from memory import traversal as _tr                      # noqa: E402
    from memory.traversal import params_for, numeric_columns   # noqa: E402
    params = params_for(cfg)
    check("параметры выведены из конфига стенда, а не из кода агента",
          len(params) > 40, f"{len(params)} записей")
    check("соответствие, которого не было в рукописном словаре, теперь есть",
          any("блок 3" in name for name in params), str(sorted(params)[:3]))
    check("обе половины берутся из одной строки конфига и разойтись не могут",
          all(spec["норма"] and spec["код"] for spec in params.values()))

    found = {stage: [c["параметр"] for c in
                     _tr.build_map(cfg, stage)["непокрытые_условия"]]
             for stage in ("экструзия", "печать", "кольцевание")}
    check("найден пропуск по диаметру кольца",
          found["кольцевание"] == ["диаметр кольца"], str(found["кольцевание"]))
    check("найден пропуск по калибровому блоку — рукописный словарь его не видел",
          any("блок 3" in p for p in found["экструзия"]), str(found["экструзия"]))
    # Ярлык — не условие расчёта. «Множество оборудования» со значениями
    # «Набор-А», «Набор-Б» код использует через содержимое группы, не обращаясь к
    # имени колонки. Ложный кандидат не безобиден: он заставляет планировщик
    # поднимать поиск по нормам и коду на посторонних вопросах.
    check("текстовые ярлыки в кандидаты не попадают",
          not found["печать"], str(found["печать"]))
    numbers = numeric_columns(cfg)
    check("числовые колонки отделены от текстовых",
          "диаметр кольца" in numbers and "множество оборудования" not in numbers,
          f"{len(numbers)} числовых")
