...
def main() -> int:
    ...
    print("\nОтсутствие записи несёт координату")
    # Живой прогон 14.09: агент установил, что заказа Z-0856 нет ни в задании, ни
    # в расчёте, но оба факта пришли без координаты — ToolNotFound её не нёс. Без
    # ссылки факт отбрасывался как неподтверждённый, и пользователю предлагали
    # назвать номер заказа, который он только что назвал.
    for tool, args in (("read_task", {"order_number": "Z-0856"}),
                       ("read_plan", {"order_number": "Z-0856"})):
        r_abs = call(tool, **args)
        check(f"{tool}: отсутствие — это not_found, а не ошибка",
              r_abs.status == "not_found", r_abs.status)
        check(f"{tool}: названо, где именно искали",
              bool(r_abs.source), repr(r_abs.source))
